...
class Model(nn.Module):
    def __init__(self, num_class=400, num_point=18, num_person=2, graph=None, graph_args=dict(), in_channels=3,
        ...
        self.cls_linear = nn.Linear(512,60,dtype=torch.float32).to('cuda') #bs 60
        ...
        self.clip_text, self.clip_text_dict = clip.load('ViT-B/16',
        ...
        pretrain=True,
        joint_st = False) 
        self.clip_text = self.clip_text.float()

        for name, param in self.clip_text.named_parameters():
            if "visual" not in name and "logit_scale" not in name:
                param.requires_grad_(False)

        self.clip_text = self.clip_text.cuda()
    ...
    def forward(self, x, y,texts):
        ...
        cls_emb, text_emb, logit_scale = self.clip_text(texts,return_token=False)
