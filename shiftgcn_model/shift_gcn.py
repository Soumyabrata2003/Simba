...
import sys
sys.path.append("./shiftgcn_model/Temporal_shift/")
...
def naive_temporal_shift(x):
    ...
    out[:, fold: 2*fold, :-2, :] = x[:, fold: 2*fold, 2:, :]  		# shift left
    out[:, 2*fold: 3*fold, :-3, :] = x[:, 2*fold: 3*fold, 3:, :]  	# shift left
    out[:, 3*fold: 4*fold, :-4, :] = x[:, 3*fold: 4*fold, 4:, :]  	# shift left

    out[:, 4*fold: 5*fold, 1:, :] = x[:, 4*fold: 5*fold, :-1, :]  	# shift right
    out[:, 5*fold: 6*fold, 2:, :] = x[:, 5*fold: 6*fold, :-2, :]  	# shift right
    out[:, 6*fold: 7*fold, 3:, :] = x[:, 6*fold: 7*fold, :-3, :]  	# shift right
    out[:, 7*fold: 8*fold, 4:, :] = x[:, 7*fold: 8*fold, :-4, :]  	# shift right

    out[:,8*fold:, :, :] = x[:,8*fold:, :, :]  				# not shift
...
class Shift_tcn(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=9, stride=1):
        ...
        self.temporal_linear = nn.Conv2d(in_channels, out_channels, kernel_size=1, padding=0,stride=(stride, 1))
        nn.init.kaiming_normal_(self.temporal_linear.weight, mode='fan_out')
        
    def forward(self, x):
        x = self.bn(x)
	    # shift1
        x = naive_temporal_shift(x)
        x = self.temporal_linear(x)
        # shift2
        x = naive_temporal_shift(x)
        x = self.relu(x)
