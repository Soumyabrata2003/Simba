...
from mamba_blocks import MambaBlock, ImgClassifier
...
class Mamba_Block(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, dropout=None, d_model: int = 768):
        super().__init__()
        if dropout is None:
        ...
        self.mamblocks = nn.Sequential(*[Mamba(d_model=self.d_model) for i in range(2*layers)])
        # blocks=[]
        # for i in range(layers):
        #     # Add ResidualAttentionBlock
        #     blocks.append(ResidualAttentionBlock(width, heads, attn_mask, dropout=dropout[i]))

        #     # Add two Mamba blocks
        #     blocks.append(Mamba(d_model=d_model))
        #     blocks.append(Mamba(d_model=d_model))

        # Create nn.Sequential with the interleaved blocks
        # self.mamblocks = nn.Sequential(*blocks)
    ...
    def forward(self, x: torch.Tensor):
        # print(self.drop)
        for r in self.mamblocks:
            if self.grad_checkpointing and not torch.jit.is_scripting():
                x = checkpoint(r, x)
                # x = self.drop(self.norm(checkpoint(r,x) + x))
            else:
                x = r(x) 
                # x = self.drop(self.norm(r(x) + x))
        return x
...
class VisualTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int,dropout = None,joint=False, emb_dropout = 0., T=8):
        ...
        # self.mamba_block = Mamba_Block(width, layers, heads, dropout=dropout, d_model=768) #768-> hidden_dim for Base
...
class Vision_Mamba(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = self.ln_post(x.to('cuda')[:, 0, :])

        if self.proj is not None:
            # self.proj = nn.Parameter(self.proj.clone().to(x.dtype))
            # print(x.dtype,self.proj.dtype)
            # x = x @ self.proj
            x= torch.matmul(x.to('cuda'),self.proj.to('cuda'))
        return x
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            # self.visual = ImgClassifier(patch_size=vision_patch_size,
                                        # img_size=image_resolution,
                                        # embed_dim=embed_dim,
                                        # n_layers=2*vision_layers)  # n_layers=2*vision_layers for mamba
...
class CLIP_text(nn.Module):
    ...
    def encode_text(self, text, return_token=False):
        # print(text.device,self.dtype)
        # print(type(self.token_embedding(text.to('cuda')).type(self.dtype)))
        x = self.token_embedding(text.to('cuda')).type(self.dtype)  # [batch_size, n_ctx, d_model]
        # print(x.dtype)
        x = x + self.positional_embedding.type(self.dtype)
        # print(x.dtype)
    ...
    def forward(self, text, return_token=False):
        # image_feats = self.encode_image(image)
        cls_feat, text_feats = self.encode_text(text, return_token)
        return cls_feat, text_feats, self.logit_scale.exp()
